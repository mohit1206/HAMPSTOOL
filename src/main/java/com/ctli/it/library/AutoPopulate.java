...
public class AutoPopulate extends PageObject {
		...
		for (IntContainerField xmlField : xmlFields) {
			if (pageFields.containsKey(xmlField.getName())) {
				...
				populateValueIntoElement(findByValue, prefix,
						xmlField.getName(), element, elementValue, page);
	...
	private void populateValueIntoElement(String findByValue, String prefix,
		...
		try {
			...
			if (elementValue.contains("waitForAjaxCallEPCToFinish")) {
				elementValue = elementValue.split(":")[1];
				System.out.println("Waiting for ajax call to finish.....");
				waitForProgressBarToCloseInEPCPage(page);
			}

			if (prefix.equals("tbx")) {
				element.typeAndTab(elementValue);
				...
				if (elementValue.contains("Index:")) {
					if (element.getSelectOptions().size() > 0) {
						...
						Assert.fail("Element " + fieldName
								+ " does not have any options loaded");
					}
				} else {
					if (element.getSelectOptions().contains(elementValue)) {
						System.out.println("Populated '" + elementValue
								+ "' in to DropDown ( '" + fieldName + " -> "
								+ findByValue + "' )");
						element.selectByVisibleText(elementValue);
					} else {
						Assert.fail("Element " + fieldName
								+ " does not have the option " + elementValue);
					}
				}
	...
	public void waitForProgressBarToCloseInEPCPage(Object page) {
		waitABit(1000);
		try {
			Field f = page.getClass().getField("btn_save");
			WebElementFacade f1 = (WebElementFacade) f.get(page);

			int j = 0;
			while (j < 1000) {
				if (f1.isCurrentlyEnabled()) {
					break;
				} else {
					waitABit(100);
					j++;
